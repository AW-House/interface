    ...
    switch (warning.level) {
      case WARNING_LEVEL.MEDIUM:
        heading = (
          <Plural
            value={plural ? 2 : 1}
            one={t(`{{name}} isn't traded on leading centralized exchanges.`, {
      ...
      case WARNING_LEVEL.UNKNOWN:
        heading = (
          <Plural
            value={plural ? 2 : 1}
            one={t(`{{name}} isn't traded on leading centralized exchanges or frequently swapped on Swap.`, {
              name: tokenSymbol ?? 'This token',
            })}
            other="These tokens aren't traded on leading centralized exchanges or frequently swapped on Swap."
          />
        )
        description = <Trans>Always conduct your own research before trading.</Trans>
        break
      case WARNING_LEVEL.BLOCKED:
        description = (
          <Plural
            value={plural ? 2 : 1}
            one={t(`You can't trade {{name}} using the Swap App.`, {
              name: tokenSymbol ?? 'this token',
            })}
